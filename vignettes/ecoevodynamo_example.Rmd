...
test <- eed_solver(mac_lev(Ns, traits, comp, K, r), jit = TRUE)
test2 <- eed_solver(mac_lev(Ns, traits, comp, K, r), jit = FALSE)

time <- bench::mark(
  res <- test$ode_fun(1,
               test$tensor_list_to_vector(list(Ns = Ns, traits = traits)),
               list(comp = comp, K = K, r = r)),
  res <- test2$ode_fun(1,
               test2$tensor_list_to_vector(list(Ns = Ns, traits = traits)),
               list(comp = comp, K = K, r = r)),
  min_time = Inf,
  max_iterations = 1000,
  check = FALSE
...
test$vector_to_tensor_list(res[[1]])

ode_test <- deSolve::ode(test$tensor_list_to_vector(list(Ns = Ns, traits = traits)),
                         c(1:50),
                         test$ode_fun,
                         list(comp = comp, K = K, r = r))
ode_test
